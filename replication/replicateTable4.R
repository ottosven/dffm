...
get.ICmin = function(cdata){
  ...
  ccriteria = dffm.criterion(cfpca, K.max=8, p.max=8)
  ccriteria$IC.min[1:3,1]
}
output = function(data){
  out=c(as.numeric(c(get.ICmin(data), dim(data)[2], colnames(data)[1], colnames(data)[dim(data)[2]])))
  names(out) = c("bic", "hqc", "fFPE", "number of maturities", "lowest maturity", "highest maturity")
  out
}
##
results = sapply(G7data, output)
colnames(results) = names(G7data)
results
