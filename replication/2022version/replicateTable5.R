...
## ####################################################################
#####################
## NOTE: This code runs about 20-30 minutes
#####################
library(dffm)
## ##################################
## Data
...
insample.prederror = function(data){
  ...
  data.fpca = fpca.preprocess(data, method = "naturalsplines", workinggrid = seq(as.numeric(colnames(data))[1],as.numeric(colnames(data))[dim(data)[2]],0.5))
...
outofsample.prederror = function(data, h=1, traininglength = 120){
  ...
  for(i in 1:length(evalperiods)){
    ...
    data.fpca = fpca.preprocess(traindata, method = "naturalsplines", workinggrid = seq(as.numeric(colnames(data))[1],as.numeric(colnames(data))[dim(data)[2]],0.5))
    IC = dffm.criterion(data.fpca)$IC.min
...
RMSFE.JKV = round(rbind(insample, out1ahead, out3head, out6ahead),3)
nnames=c(
...
rownames(RMSFE.JKV) = c("in-sample 1-step", "1-ahead", "3-ahead", "6-ahead")
RMSFE.JKV
## ##################################
## Results for fed
## ##################################
...
RMSFE.fed = round(rbind(insample, out1ahead, out3head, out6ahead),3)
nnames=c(
  "VAR-BIC", "VAR-HQC", "AR-BIC", "AR-HQC",
  paste("VAR_",c(3,4,6),"(1)",sep=""),
  paste("AR_",c(3,4,6),"(1)",sep=""),
...
rownames(RMSFE.fed) = c("in-sample 1-step", "1-ahead", "3-ahead", "6-ahead")
RMSFE.fed


write.table(RMSFE.JKV,file=paste("./table5a.csv", sep=''),append=T,col.names=F,row.names=F)
write.table(RMSFE.fed,file=paste("./table5b.csv", sep=''),append=T,col.names=F,row.names=F)
